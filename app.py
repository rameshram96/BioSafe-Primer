...
.section-header{border-left:4px solid #0072B2;padding-left:12px;color:#0072B2;font-size:1.1rem;font-weight:600;margin:20px 0 12px;}
.upload-done{background:#E8F5E9;border:1.5px solid #009E73;border-radius:8px;padding:12px 16px;color:#1B4332;font-size:13px;margin-bottom:10px;}
.primer-code{font-family:'Source Code Pro','Courier New',monospace !important;background:#EEF2F7;border-radius:6px;
  padding:6px 10px;font-size:12.5px;color:#1C1C1E;margin:4px 0;border-left:3px solid #0072B2;}
...
.footer-ribbon{position:fixed;bottom:0;left:0;right:0;background:#0072B2;color:#CDEAF8;
  ...
  letter-spacing:.3px;box-shadow:0 -2px 8px rgba(0,114,178,.18);
...
def footer():
    st.markdown('<div class="footer-ribbon"><strong>BioSafe Primer</strong> &nbsp;·&nbsp; '
                'Developed and maintained by <strong>Division of Plant Physiology, '
                'ICAR-Indian Agricultural Research Institute</strong></div>',
                unsafe_allow_html=True)
...
def ov_badge(val, min_ov=50):
    ...
    if val >= min_ov: return f'<span class="ov-badge ov-ok">✔ {val} bp</span>'
...
def _redesign_ui(p):
    ...
    if pk in st.session_state:
        np      = st.session_state[pk]['primer']
        ext_l   = st.session_state[pk]['ext_l']
        ext_r   = st.session_state[pk]['ext_r']
        reason  = st.session_state[pk]['reason']
        ft      = st.session_state[pk]['failure_type']
        attempt = st.session_state[pk]['attempt']
        ...
        viol_html = ('<div style="margin-top:10px;background:#FDECEA;border-radius:6px;padding:8px 10px;">'
                     + "".join(f'<div style="margin:3px 0;font-size:11.5px;color:#D55E00">⚠ {v}</div>' for v in viol_list)
                     + '</div>') if viol_list else (
                     '<div style="margin-top:10px;background:#D4EDDA;border-radius:6px;'
                     'padding:8px 10px;font-size:11.5px;color:#155724">✅ All rules pass — safe to accept.</div>')
        rules_html = " &nbsp;|&nbsp; ".join([
  ...
  <div style="font-size:12px;margin-bottom:10px">
    ...
    <strong>Length:</strong> {np['amplicon_length']} bp | <strong>Pair Penalty:</strong> {np.get('pair_penalty',0)}</div>
  <div style="display:flex;gap:12px;align-items:center;margin-bottom:5px">
    <span style="font-size:12.5px;font-weight:600;color:#3A4A5C">🔼 Upstream (Amp {np['amplicon_num']-1}):</span>
    {ov_badge(prev_ov)}</div>
  <div style="display:flex;gap:12px;align-items:center">
    <span style="font-size:12.5px;font-weight:600;color:#3A4A5C">🔽 Downstream (Amp {np['amplicon_num']+1}):</span>
    {ov_badge(next_ov)}</div>
  {viol_html}
</div>""", unsafe_allow_html=True)
        ca, cb = st.columns(2)
        if ca.button("✅ Accept & Save", key=f"acc_{p['_id']}", type="primary", disabled=not all_ok):
            np['status'] = 'Pending'; np['amplicon_name'] = amp_label(p)
            ...
            add_redesign_history(proj(), p['amplicon_num'], p['_id'],
                ext_l, ext_r, reason, ft, attempt, np.get('overlap_prev'), np.get('overlap_next'))
            del st.session_state[pk]
            st.success("✅ Saved! Remember to 💾 Save Project.")
            st.rerun()
        if cb.button("❌ Reject & Try Again", key=f"rej_{p['_id']}"):
            del st.session_state[pk]; st.rerun()
        if not all_ok: st.error("⛔ Fix rule violations before accepting.")
        return

    if p.get('version',1) >= MAX_REDESIGN_VERSIONS:
        st.error(f"⛔ Maximum {MAX_REDESIGN_VERSIONS} redesign attempts reached."); return

    FOPTS = ['No band','Multiple bands','Weak band','Wrong band size','Primer dimer','Overlap violation','Other']
    ...
    if ext_r > 0:
        st.warning("⚠️ Downstream extension does not guarantee downstream overlap. Check the preview carefully.")
    reason = st.text_input("Additional notes (optional)", placeholder="e.g. No band after 35 cycles", key=f"rsn_{p['_id']}")
    ...
    if st.button("🔬 Run Redesign", key=f"red_{p['_id']}", type="primary"):
        ...
        with st.spinner("Redesigning with Primer3…"):
            new_p, err = redesign_primers(seq, p['amplicon_start'], p['amplicon_end'],
        ...
        if new_p:
            st.session_state[f"rdp_{p['_id']}"] = {
                ...
                'reason':reason,'failure_type':ft,'attempt':p.get('version',1)+1}
            st.rerun()
        else:
...
if 'project' not in st.session_state:
    ...
    with c1: new_name = st.text_input("Project name", placeholder="e.g. pCAMBIA1300_GFP")
    with c2:
...
st.markdown(f"""<div class="proj-banner">
  ...
  {proj().get('vector_length',0):,} bp &nbsp;·&nbsp; Created {proj().get('created_at','—')[:16]}</span></div>
</div>""", unsafe_allow_html=True)

ba, bb = st.columns([2,2])
with ba:
    if st.button("← Close Project"):
        del st.session_state['project']; st.session_state.pop('seq_info',None); st.rerun()
with bb:
...
with tab2:
    ...
    if not best: st.info("No primers designed yet.")
        ...
        st.caption("💡 Click any amplicon for details | ESC to close | Show Sequence → Ctrl+F to search")
...
with tab3:
    ...
    for p in best:
        label = amp_label(p)
        with st.expander(f"{label}  |  v{p['version']}  |  {p['amplicon_start']}–{p['amplicon_end']} bp  |  {p['status']}"):
            ...
            with c1:
                ...
                st.markdown(f"**Pair Penalty:** {p.get('pair_penalty',0)} &nbsp;|&nbsp; **Amp:** {p['amplicon_length']} bp &nbsp;|&nbsp; "
                            f"🔼 Upstream: {ov_badge(p.get('overlap_prev'))} &nbsp; 🔽 Downstream: {ov_badge(p.get('overlap_next'))}",
                            unsafe_allow_html=True)
            with c2:
                valid_s = ['Pending','Done','Failed','Overlap Violation']
                cur = p['status'] if p['status'] in valid_s else 'Pending'
                new_s = st.selectbox("Status", valid_s, index=valid_s.index(cur), key=f"st_{p['_id']}")
...
with tab4:
    ...
    if not popts:
        ...
        if st.button("💾 Save Run", type="primary"):
            ...
            update_primer_status(proj(), g_p['_id'], "Done" if g_rs=="Pass" else "Failed")
...
with tab5:
    ...
    st.caption("The .bsp file contains everything: primers, gel images (embedded), PCR runs, redesign history.")
    ...
    with c2:
        if st.button("📄 Generate PDF Report"):
            ...
            st.download_button("⬇️ Download PDF Report", data=buf,
